...
const CategoryItem: React.FC<Props> = ({ icon, index }) => {
  ...
  return (
    <Link
      ...
      className="w-full md:w-1/6 bg-[#D9D9D9] h-28 rounded-full flex items-center justify-center text-white border-2 relative overflow-hidden cursor-pointer"
    >
      <motion.svg
        key={index}
      ...
      >
        <Image src={icon.src} width={300} height={300} alt=".." />
      </motion.div>
    </Link>
  );
};
